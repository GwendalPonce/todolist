 ...
 * - on veut faire disparaitre l'index.php 
 * - on ne veut pas voir le chemin physique des dossiers du site
 * - on veut avoir de "jolies" URLs de type: monsite.fr/ma-page
 * 
 ...
 * 
 * Il existe une alternative (uniquement en phase de développement) : au lieu d'utiliser Apache en local, on va utiliser le serveur web interne de PHP
 *  on va utiliser le serveur web interne de PHP (PHP built-in web server)
 * 
 * Pour démarrer le serveur web de PHP, dans le terminal on se place dans le dossier du projet et on lance la commande suivante :
 * 
 ...
 *   Page d'accueil : /
 *   Page création d'une tache : /add-task
 */
...
// On va chercher la route correspondant a la page sur laquelle on se trouve
$controller = null;
...
// Si $controller est null, la route n'existe pas donc => erreur 404
if ($controller == null){
